...
async def test_send_reminder_notification_with_previous_message_deletion(
    ...
    mock_bot.delete_message.assert_called_once_with(
        chat_id=reminder.chat_id, message_id=456
    )
...
@pytest.mark.asyncio
async def test_build_notification_text_first_attempt(
...
@pytest.mark.asyncio
async def test_build_notification_text_escalated(
    notification_service, populated_database
):
    reminder = populated_database["reminder"]
...
@pytest.mark.asyncio
async def test_build_notification_text_weekly_reminder(
    notification_service, populated_database
):
    reminder = populated_database["reminder"]
...
@pytest.mark.asyncio
async def test_send_escalation_warning_success(
    notification_service, populated_database, mock_bot
):
    reminder = populated_database["reminder"]
    reminder_dto = entity_to_reminder_dto(reminder)
...
@pytest.mark.asyncio
async def test_send_escalation_warning_fails(
    notification_service, populated_database, mock_bot
):
    reminder = populated_database["reminder"]
    reminder_dto = entity_to_reminder_dto(reminder)
...
async def test_calculate_next_notification_interval_escalating(notification_service):
    ...
    interval_0 = notification_service.calculate_next_notification_interval(
        0, base_interval
    )
    interval_1 = notification_service.calculate_next_notification_interval(
        1, base_interval
    )
    interval_3 = notification_service.calculate_next_notification_interval(
        3, base_interval
    )
    interval_5 = notification_service.calculate_next_notification_interval(
        5, base_interval
    )
...
async def test_handle_notification_response_confirm(
    ...
    mock_reminder_service.get_reminder_by_id.return_value = entity_to_reminder_dto(
        reminder
    )
    ...
    mock_reminder_service.confirm_reminder.assert_called_once_with(
        reminder.id, reminder.user_id, None
    )
...
async def test_handle_notification_response_unauthorized(
    ...
    mock_query.answer.assert_called_once_with(
        "Reminder not found or access denied.", show_alert=True
    )


@pytest.mark.asyncio
async def test_handle_confirmation_passes_job_scheduler(
    notification_service, populated_database, mock_bot
):
    reminder = populated_database["reminder"]
    ...
    await notification_service.handle_notification_response(
        mock_query, mock_reminder_service, mock_job_scheduler
    )

    mock_reminder_service.confirm_reminder.assert_called_once_with(
        reminder.id, reminder.user_id, mock_job_scheduler
    )
