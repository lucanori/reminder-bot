...
class NotificationService:
    ...
    async def send_reminder_notification(
        ...
        try:
            if reminder.last_message_id:
                try:
                    await self.bot.delete_message(
                        chat_id=reminder.chat_id, message_id=reminder.last_message_id
                    )
            ...
            keyboard = [
                [
                    InlineKeyboardButton(
                        "✅ Completed", callback_data=f"confirm_{reminder.id}"
                    ),
                    InlineKeyboardButton(
                        "⏰ Snooze 5min", callback_data=f"snooze_{reminder.id}"
                    ),
                ]
            ]
            ...
            try:
                await self.reminder_repo.update_message_id(
                    reminder.id, message.message_id
                )
            except Exception as e:
                logger.warning(
                    "failed_to_update_message_id", reminder_id=reminder.id, error=str(e)
                )
            ...
            logger.error("notification_failed", reminder_id=reminder.id, error=str(e))
            raise TelegramAPIException(f"Failed to send notification: {e}")

    async def handle_notification_response(
        self, callback_query: CallbackQuery, reminder_service, job_scheduler=None
    ) -> None:
        try:
            ...
            if action == "confirm":
                ...
                await self._handle_confirmation(
                    callback_query, reminder, reminder_service, job_scheduler
                )
            elif action == "snooze":
                await callback_query.answer()
        ...
        if success:
            ...
            try:
                await callback_query.edit_message_text(
                    text=completion_text, parse_mode="HTML"
                )
                logger.info(
                    "reminder_confirmed",
                    reminder_id=reminder.id,
    ...
    async def _handle_snooze(
        self, callback_query: CallbackQuery, reminder: ReminderDTO, reminder_service
    ) -> None:
        snooze_minutes = 5
        ...
        if success:
            snooze_text = (
                f"⏰ <b>Snoozed for {snooze_minutes} minutes</b>\n\n{reminder.text}"
            )

            try:
                await callback_query.edit_message_text(
                    text=snooze_text, parse_mode="HTML"
                )
                logger.info(
                    "reminder_snoozed",
                    reminder_id=reminder.id,
    ...
    async def send_escalation_warning(self, reminder: ReminderDTO) -> bool:
        try:
            ...
            await self.bot.send_message(
                chat_id=reminder.chat_id, text=warning_text, parse_mode="HTML"
            )
            ...
            logger.error(
                "escalation_warning_failed", reminder_id=reminder.id, error=str(e)
            )
            return False

    def calculate_next_notification_interval(
        self, current_count: int, base_interval: int
    ) -> int:
